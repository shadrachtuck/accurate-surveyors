jQuery( document ).ready( function( $ ) {
	...
	var blockchain_initialize_widget = function ( widget_el ) {
		blockchain_repeating_sortable_init( widget_el );
		blockchain_colorpicker_init( widget_el );
		blockchain_alpha_colorpicker_init( widget_el );
		blockchain_collapsible_init( widget_el );
	};

	blockchain_initialize_widget();

	function blockchain_on_customizer_widget_form_update( e, widget_el ) {
		blockchain_initialize_widget( widget_el );
	}
	...
	// $( document ).on( 'widget-updated', blockchain_on_customizer_widget_form_update );
	$( document ).on( 'widget-added', blockchain_on_customizer_widget_form_update );
	...
	$( document ).ajaxSuccess( function( e, xhr, options ) {
		if ( options.data.search( 'action=save-widget' ) != -1 ) {
			...
			if ( ( widget_id = options.data.match( /widget-id=(ci-.*?-\d+)\b/ ) ) !== null ) {
				...
				blockchain_initialize_widget( widget );
			}
		}
	} );
	...
	$body.on( 'change', ':has(input.id_base[value="ci-home-post-type-items"]) .blockchain-post-type-select', function() {
...
var blockchain_collapsible_init = function( selector ) {
...
var blockchain_alpha_colorpicker_init = function( selector ) {
	if ( selector === undefined ) {
		var blockchain_AlphaColorPicker = jQuery( '#widgets-right .blockchain-alpha-color-picker, #wp_inactive_widgets .blockchain-alpha-color-picker' ).filter( function() {
			return !jQuery( this ).parents( '.field-prototype' ).length;
		} );

		blockchain_AlphaColorPicker.alphaColorPicker();
	} else {
		jQuery( '.blockchain-alpha-color-picker', selector ).alphaColorPicker();
	}
};

var blockchain_colorpicker_init = function( selector ) {
	if ( selector === undefined ) {
		var blockchain_ColorPicker = jQuery( '#widgets-right .blockchain-color-picker, #wp_inactive_widgets .blockchain-color-picker' ).filter( function() {
			return !jQuery( this ).parents( '.field-prototype' ).length;
		} );
		...
		blockchain_ColorPicker.each( function() {
		...
		jQuery( '.blockchain-color-picker', selector ).each( function() {
			jQuery( this ).wpColorPicker( {
				change: _.throttle( function () {
					jQuery( this ).trigger( 'change' );
				}, 1000, { leading: false } )
