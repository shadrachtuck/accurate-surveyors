<?php
	...
	if ( ! $support['required'] ) {
		...
		$wp_customize->add_control( 'hero_show', array(
			...
			'label'   => esc_html__( 'Show hero', 'blockchain' ),
		) );
	}
	...
	$wp_customize->add_control( 'hero_video_url', array(
		'type'        => 'url',
		'section'     => 'theme_colors_hero',
		'label'       => esc_html__( 'Video URL', 'blockchain' ),
		'description' => esc_html__( 'Video is only displayed on desktops. Make sure you also set a background image for optimal result in mobile devices. Supported video providers: YouTube and Vimeo.', 'blockchain' ),
	) );


	// Needed by the hero image descriptions.
	global $_wp_additional_image_sizes;
	$size      = $_wp_additional_image_sizes['blockchain_hero'];
	...
	$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'hero_image', array(
		'section'     => 'theme_colors_hero',
		'label'       => esc_html__( 'Hero image', 'blockchain' ),
		/* translators: %s is an image dimension pair in the form "width x height" without spaces, e.g. 800x600 */
		'description' => esc_html( sprintf( __( 'Recommended size of at least %s pixels.', 'blockchain' ), $hero_size ) ),
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'hero_bg_color', array(
		...
		'description' => esc_html__( 'Select a color for your hero section. This will not be visible if you set a hero image, unless the image itself has transparent areas.', 'blockchain' ),
		'section'     => 'theme_colors_hero',
	) ) );
	...
	$wp_customize->add_setting( 'hero_overlay_color', array(
		'transport'         => 'postMessage',
		'default'           => '',
		'sanitize_callback' => 'blockchain_sanitize_rgba_color',
	) );
	$wp_customize->add_control( new Customize_Alpha_Color_Control( $wp_customize, 'hero_overlay_color', array(
		'label'        => esc_html__( 'Overlay color', 'blockchain' ),
		'description'  => wp_kses( __( 'Select an overlay color for your hero section. This will be visible <strong>on top</strong> of the image, so adjust the opacity to achieve the required result and reveal the hero image.', 'blockchain' ), blockchain_get_allowed_tags() ),
		'section'      => 'theme_colors_hero',
		'show_opacity' => true,
	) ) );
	...
	$wp_customize->selective_refresh->add_partial( 'theme_hero', array(
		'selector'            => '.page-hero',
		'render_callback'     => 'blockchain_customize_preview_hero',
		'settings'            => array(
			'hero_video_url',
		),
