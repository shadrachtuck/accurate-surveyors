<?php
	 ...
	 * @return string
	 */
	function blockchain_get_customizer_css() {
		ob_start();
		...
		if ( apply_filters( 'blockchain_customizable_header', true ) ) {
			...
			$header_primary_menu_bg_image      = get_theme_mod( 'header_primary_menu_bg_image' );
			$header_primary_menu_bg_repeat     = get_theme_mod( 'header_primary_menu_bg_repeat', 'no-repeat' );
			$header_primary_menu_bg_position_x = get_theme_mod( 'header_primary_menu_bg_image_position_x', 'center' );
			$header_primary_menu_bg_position_y = get_theme_mod( 'header_primary_menu_bg_image_position_y', 'center' );
			$header_primary_menu_bg_attachment = get_theme_mod( 'header_primary_menu_bg_image_attachment', 'scroll' );
			$header_primary_menu_bg_cover      = get_theme_mod( 'header_primary_menu_bg_image_cover', 1 );

			if ( ! empty( $header_primary_menu_bg_color ) || ! empty( $header_primary_menu_bg_image ) ) {
				?>
				.head-mast,
				.head-sticky.is_stuck {
					<?php
						if ( $header_primary_menu_bg_color ) {
							printf( 'background-color: %s; ', sanitize_hex_color( $header_primary_menu_bg_color ) );
						}

						if ( $header_primary_menu_bg_image ) {
							printf( 'background-image: url(%s); ', esc_url_raw( $header_primary_menu_bg_image ) );

							if ( $header_primary_menu_bg_repeat ) {
								printf( 'background-repeat: %s; ', blockchain_sanitize_image_repeat( $header_primary_menu_bg_repeat ) );
							}

							if ( $header_primary_menu_bg_position_x && $header_primary_menu_bg_position_y ) {
								printf( 'background-position: %s %s; ',
									blockchain_sanitize_image_position_x( $header_primary_menu_bg_position_x ),
									blockchain_sanitize_image_position_y( $header_primary_menu_bg_position_y )
								);
							}

							if ( $header_primary_menu_bg_attachment ) {
								printf( 'background-attachment: %s; ', blockchain_sanitize_image_attachment( $header_primary_menu_bg_attachment ) );
							}

							if ( ! $header_primary_menu_bg_cover ) {
			...
			$header_primary_menu_text_on_bg_color = get_theme_mod( 'header_primary_menu_text_on_bg_color' );

			if ( ! empty( $header_primary_menu_text_on_bg_color ) ) {
				?>
				.head-mast.over-background,
				.over-background .site-logo a,
				.over-background .site-tagline,
				.over-background .navigation-main > li > a {
					color: <?php echo sanitize_hex_color( $header_primary_menu_text_on_bg_color ); ?>;
				}
				<?php
			}
			...
			if ( ! empty( $header_primary_menu_active_color ) ) {
				...
				.navigation-main .nav-button > a:hover {
					background-color: <?php echo sanitize_hex_color( $header_primary_menu_active_color ); ?>;
					color: #fff;
				}
			...
			if ( ! empty( $header_primary_submenu_active_text_color ) ) {
				...
				.navigation-main li .current-menu-ancestor > a {
					background-color: <?php echo sanitize_hex_color( $header_primary_submenu_active_text_color ); ?>;
				}
				<?php
			}
		...
		if ( ! empty( $site_accent_color ) ) {
			...
			a:hover {
				color: <?php echo sanitize_hex_color( blockchain_color_luminance( $site_accent_color, .1 ) ); ?>;
			}
			...
			.blockchain-slick-slider .slick-dots .slick-active button {
				border-color: <?php echo sanitize_hex_color( $site_accent_color ); ?>;
				box-shadow: 0 2px 0 <?php echo sanitize_hex_color( blockchain_hex2rgba( $site_accent_color, .2 ) ); ?>;
			}
			<?php
		}
		...
		if ( ! empty( $site_text_color ) ) {
			$site_text_color_dark = blockchain_color_luminance( $site_text_color, -0.5 );
			?>
			body,
			blockquote cite,
			.instagram-pics li a,
		...
		$css = ob_get_clean();
		return apply_filters( 'blockchain_customizer_css', $css );
	}

	function blockchain_get_hero_styles() {
		$hero  = blockchain_get_hero_data();
		$style = '';

		if ( ! $hero['show'] ) {
			return apply_filters( 'blockchain_hero_styles', $style, $hero );
		}
		...
		$styles_selector  = apply_filters( 'blockchain_hero_styles_selector', $styles_selector );
		$overlay_selector = apply_filters( 'blockchain_hero_styles_overlay_selector', $overlay_selector );
	...
	if ( ! function_exists( 'blockchain_get_all_customizer_css' ) ) :
		function blockchain_get_all_customizer_css() {
			$styles = array(
				'customizer' => blockchain_get_customizer_css(),
				'hero'       => blockchain_get_hero_styles(),
			);

			$styles = apply_filters( 'blockchain_all_customizer_css', $styles );
