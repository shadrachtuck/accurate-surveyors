...
if ( ! class_exists( 'Blockchain_Onboarding_Page' ) ) {

	/**
	 * Class used to generate the onboarding page.
	 */
	class Blockchain_Onboarding_Page {
		...
		public function themedata_setup() {
			...
			if ( is_child_theme() ) {
				...
				if ( $parent_theme && is_object( $parent_theme ) ) {
					$this->theme_name = $parent_theme->get( 'Name' );
					$this->theme      = $parent_theme;
				} else {
					// Fallback to current theme if parent is not available
		...
		public function page_setup() {
			...
			add_action( 'wp_ajax_blockchain_dismiss_onboarding', array( $this, 'dismiss_onboarding' ) );
			add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_admin_styles' ) );
			add_action( 'admin_menu', array( $this, 'register' ) );
			add_action( 'wp_ajax_install_blockchain_plugin', array( $this, 'install_plugin' ) );
		}
		...
		public function redirect_to_onboarding() {
			...
			if ( is_admin() && 'themes.php' === $pagenow && isset( $_GET['activated'] ) && current_user_can( 'manage_options' ) ) {
				wp_redirect( admin_url( 'themes.php?page=' . $this->theme_slug . '-onboard' ) );
				exit;
			}
		}
		...
		public function onboarding_notice() {
			...
			<div class="notice notice-info is-dismissible">
				<div class="blockchain-onboarding-notice">
					<p>
						<?php
							/* translators: %1$s is the theme name. %2$s URL to onboarding page. */
							echo wp_kses( sprintf( __( 'Welcome to %1$s. Check out the <a href="%2$s">onboarding page</a> to get things started.', 'blockchain' ),
								$this->theme_name,
			...
			wp_enqueue_script( 'blockchain-onboarding-notice', get_template_directory_uri() . '/inc/onboarding/js/onboarding-notice.js', array(), $this->theme_version, true );

			$settings = array(
				'ajaxurl'       => admin_url( 'admin-ajax.php' ),
				'dismiss_nonce' => wp_create_nonce( 'blockchain-dismiss-onboarding' ),
			);
			wp_localize_script( 'blockchain-onboarding-notice', 'blockchain_Onboarding', $settings );
		}
		...
		public function dismiss_onboarding() {
			check_ajax_referer( 'blockchain-dismiss-onboarding', 'nonce' );
		...
		public function enqueue_admin_styles() {
			...
			wp_enqueue_style( 'blockchain-onboarding', get_template_directory_uri() . '/inc/onboarding/css/onboarding-styles.css', array(), $this->theme_version );

			wp_enqueue_script( 'blockchain-onboarding', get_template_directory_uri() . '/inc/onboarding/js/onboarding.js', array(
				'plugin-install',
				'updates',
			), $this->theme_version, true );

			wp_localize_script(
				'blockchain-onboarding', 'blockchain_onboarding', array(
					'onboarding_nonce'   => wp_create_nonce( 'onboarding_nonce' ),
					'ajaxurl'            => admin_url( 'admin-ajax.php' ),
					'template_directory' => get_template_directory_uri(),
					'activating_text'    => esc_html__( 'Activating', 'blockchain' ),
					'activate_text'      => esc_html__( 'Activate', 'blockchain' ),
					'installing_text'    => esc_html__( 'Installing...', 'blockchain' ),
				)
			);
		}
		...
		public function render_page() {
			...
			if ( defined( 'BLOCKCHAIN_WHITELABEL' ) && BLOCKCHAIN_WHITELABEL ) {
				...
				$logo_url = ! empty( $this->data['logo_url'] ) ? $this->data['logo_url'] : 'https://www.cssigniter.com/themes/' . BLOCKCHAIN_NAME . '/';
			}
			?>
			<div class="wrap about-wrap blockchain-onboarding-wrap full-width-layout">
				<h1><?php echo esc_html( $title ); ?></h1>

				<?php if ( ! empty( $this->data['description'] ) ) : ?>
					<p class="about-text"><?php echo wp_kses( $this->data['description'], blockchain_get_allowed_tags( 'guide' ) ); ?></p>
				<?php endif; ?>
		...
		public function required_plugins() {
			?><h3><?php esc_html_e( 'The following plugins are required. Please install and activate them.', 'blockchain' ); ?></h3><?php

			if ( ! current_user_can( 'install_plugins' ) ) :
				?><p><?php echo wp_kses( __( 'You do not have sufficient permissions to install plugins, please contact your administrator.', 'blockchain' ), blockchain_get_allowed_tags( 'guide' ) ); ?></p><?php
				return false;
			endif;
		...
		public function recommended_plugins() {
			?><h3><?php
				...
				echo esc_html( sprintf( __( 'The following plugins are not required but they will provide additional functionality to %s.', 'blockchain' ), $this->theme_name ) );
			?></h3><?php
		...
		public function sample_content() {
			$check = $this->get_plugin_action( array( 'one-click-demo-import' => __( 'One Click Demo Import', 'blockchain' ) ) );

			?><h3><?php esc_html_e( 'Import our sample content.', 'blockchain' ); ?></h3><?php

			if ( in_array( $check['one-click-demo-import'], array( 'install-plugin', 'activate' ), true ) ) {
				?>
				<div class="blockchain-onboarding-box blockchain-onboarding-box-warning">
					<h4 class="box-title"><?php esc_html_e( 'Please note:', 'blockchain' ); ?></h4>
					<p><?php esc_html_e( 'You need to install and activate One Click Demo Import before proceeding.', 'blockchain' ); ?></p>
					<p><a class="button button-primary" href="<?php echo esc_url( get_admin_url( '', 'themes.php?page=' . $this->theme_slug . '-onboard' ) . '&tab=recommended_plugins' ); ?>"><?php esc_html_e( 'Click here to do it now.', 'blockchain' ); ?></a></p>
				</div>
				<?php
			} else {
				?>
				<div class="blockchain-onboarding-box blockchain-onboarding-box-success">
					<h4 class="box-title"><?php esc_html_e( 'Good to go!', 'blockchain' ); ?></h4>
					<p><?php esc_html_e( 'Now you can import the sample content and have your theme set up like the demo using the One Click Demo Import Plugin.', 'blockchain' ); ?></p>
					<p><a class="button button-primary" href="<?php echo esc_url( get_admin_url( '', 'themes.php?page=pt-one-click-demo-import' ) ); ?>"><?php esc_html_e( 'Get Started', 'blockchain' ); ?></a></p>
				</div>
				<?php
			}
		...
		public function support() {
			...
			?>
			<h3><?php esc_html_e( 'Here are a few useful links to get you started.', 'blockchain' ); ?></h3>
			<div class="three-col">
				<?php
					...
					foreach ( $sections as $section_id => $section ) {
						?>
						<div class="col">
							<div class="blockchain-onboarding-box blockchain-support-tab-box-<?php echo esc_attr( $section_id ); ?>">
								<h4 class="box-title"><a href="<?php echo esc_url( $section['link_url'] ); ?>" target="_blank"><?php echo esc_html( $section['title'] ); ?></a></h4>
								<?php if ( ! empty( $section['description'] ) ) : ?>
									<p><?php echo wp_kses( $section['description'], blockchain_get_allowed_tags( 'guide' ) ); ?></p>
								<?php endif; ?>
		...
		public function get_plugin_boxes( $plugins, $actions ) {
			foreach ( $actions as $slug => $action ) {
				if ( 'upload-plugin' === $action ) {
					...
					<div class="col">
						<div class="blockchain-onboarding-box blockchain-onboarding-box-warning">
							...
							<?php if ( ! empty( $plugins[ $slug ]['description'] ) ) : ?>
								<p class="box-description"><?php echo wp_kses( $plugins[ $slug ]['description'], blockchain_get_allowed_tags( 'guide' ) ); ?></p>
							<?php endif; ?>
							...
							<div class="plugin-card-<?php echo esc_attr( $slug ); ?>">
								...
								<?php
									...
									echo esc_html( sprintf( __( 'The %s plugin was not found, click to install and activate.', 'blockchain' ), $plugins[ $slug ]['title'] ) );
								?>
								</p>
								<p><a href="<?php echo esc_url( $link ); ?>" class="install-now button ajax-install-plugin" data-plugin-slug="<?php echo esc_attr( $slug ); ?>"><?php esc_html_e( 'Install &amp; activate ', 'blockchain' ); ?></a></p>
							</div>	
						</div>
					</div>	
					<?php
					...
					<div class="col">
						<div class="blockchain-onboarding-box blockchain-onboarding-box-warning">
							...
							<div class="plugin-card-<?php echo esc_attr( $slug ); ?>">
								...
								<p><a data-slug="<?php echo esc_attr( $slug ); ?>" href="<?php echo esc_url( $link ); ?>" class="install-now button"><?php esc_html_e( 'Install &amp; activate ', 'blockchain' ); ?></a></p>
							</div>	
						</div>
					</div>	
					<?php
					...
					?>
					<div class="col">
						<div class="blockchain-onboarding-box blockchain-onboarding-box-info">
							<h4 class="box-title"><?php echo esc_html( $plugins[ $slug ]['title'] ); ?></h4>
							...
							<div class="plugin-card-<?php echo esc_attr( $slug ); ?>">
								...
								<?php
									/* translators: %s is the plugin name. */
									echo esc_html( sprintf( __( 'The %s plugin is installed but not active, click to activate.', 'blockchain' ), $plugins[ $slug ]['title'] ) );
								?>
								</p>
								<p><a data-slug="<?php echo esc_attr( $slug ); ?>" href="<?php echo esc_url( $link ); ?>" class="activate-now button button-primary"><?php esc_html_e( 'Activate ', 'blockchain' ); ?></a></p>
							</div>	
						</div>
					</div>
					<?php
					...
					<div class="col">
						<div class="blockchain-onboarding-box blockchain-onboarding-box-success">
							...
							<?php
								/* translators: %s is the plugin name. */
								echo esc_html( sprintf( __( '%s is installed and activated.', 'blockchain' ), $plugins[ $slug ]['title'] ) );
							?>
							</p>
						</div>
					</div>	
		...
		public function install_plugin() {
			if ( ! current_user_can( 'upload_plugins' ) ) {
				wp_die( __( 'You do not have sufficient permissions to install plugins on this site.', 'blockchain' ) );
			}

			// Verify nonce.
			if ( ! isset( $_POST['onboarding_nonce'] ) || ! wp_verify_nonce( $_POST['onboarding_nonce'], 'onboarding_nonce' ) ) {
			...
			$title    = sprintf( __( 'Installing Plugin from URL: %s', 'blockchain' ), esc_html( $plugin_source_url ) );
		...
		public function default_data() {
			...
			return array(
				...
				// Required. The logo's link URL. Defaults to 'https://www.cssigniter.com/themes/' . BLOCKCHAIN_NAME . '/'
