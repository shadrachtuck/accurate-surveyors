...
class Thrive_2FA_Updater
{
    ...
    {
        $options = get_option('thrive_2FA_settings', []);
        $this->github_username = $options['github_username'] ?? 'glossyit';
        $this->github_repo = $options['github_repo'] ?? 'Thrive2FAPlugin';
        $this->current_version = defined('WP_2FA_VERSION') ? WP_2FA_VERSION : '1.0.0';

        // Define plugin basename (path relative to plugins dir)
        $this->plugin_basename = plugin_basename(WP_2FA_PLUGIN_DIR . 'wp-2factor-auth.php');
        ...
        $this->main_file = basename(WP_2FA_PLUGIN_DIR . 'wp-2factor-auth.php');
    ...
    public function init()
    {
        ...
        if (!wp_next_scheduled('thrive_2FA_check_for_updates')) {
            wp_schedule_event(time(), 'daily', 'thrive_2FA_check_for_updates');
        }

        add_action('thrive_2FA_check_for_updates', [$this, 'trigger_update_check']);
    ...
    public function github_username_callback()
    {
        ...
        echo "<input type='text' name='thrive_2FA_settings[github_username]' value='{$val}' class='regular-text'>";
    ...
    public function github_repo_callback()
    {
        ...
        echo "<input type='text' name='thrive_2FA_settings[github_repo]' value='{$val}' class='regular-text'>";
    ...
    public function github_token_callback()
    {
        ...
        echo '<input type="password" id="github_token" name="thrive_2FA_settings[github_token]" value="' . esc_attr($github_token) . '" class="regular-text">';
    ...
    public function auto_update_callback()
    {
        ...
        echo "<input type='checkbox' name='thrive_2FA_settings[auto_update]' value='1' {$checked}>";
    ...
    private function perform_auto_update($download_url)
    {
        try {
            ...
            $extracted_dirs = glob($plugin_dir . '/glossyit-Thrive2FAPlugin-*', GLOB_ONLYDIR);
            if (empty($extracted_dirs)) {
                throw new Exception('No extracted subdirectory found matching "glossyit-Thrive2FAPlugin-*"');
            }
            ...
            error_log('Thrive 2FA auto-update completed and plugin reactivated.');

        } catch (Exception $e) {
            error_log('[Thrive 2FA Auto-Update Error] ' . $e->getMessage());
        }
    }
    ...
    private function thrive_download_zip($zip_url)
    {
        try {
            ...
            $file_name = 'Thrive-2FA-plugin.zip';
            ...
            error_log('[Thrive 2FA Download Error] ' . $e->getMessage());
            return false;
        }
    }

    /**
     * Fetch GitHub token from API
     */
    ...
    public static function deactivate()
    {
        ...
        wp_clear_scheduled_hook('thrive_2FA_check_for_updates');
    }
}
