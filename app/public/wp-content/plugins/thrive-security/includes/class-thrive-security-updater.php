...
class Thrive_Security_Updater
{
    ...
    {
        try {
            $options = get_option('thrive_security_settings', []);
            $this->github_username = $options['github_username'] ?? 'glossyit';
            $this->github_repo = $options['github_repo'] ?? 'ThriveSecurityPlugin';
            $this->current_version = defined('THRIVE_SECURITY_VERSION') ? THRIVE_SECURITY_VERSION : '1.0.0';

            // Define plugin basename (path relative to plugins dir)
            $this->plugin_basename = plugin_basename(THRIVE_SECURITY_PATH . 'thrive-security.php');

            // Plugin slug is the folder name of plugin (e.g. thrive-security-plugin)
            $this->plugin_slug = dirname($this->plugin_basename);

            // Main plugin file name inside plugin folder (e.g. thrive-security.php)
            $this->main_file = basename(THRIVE_SECURITY_PATH . 'thrive-security.php');

            $this->init();
        } catch (Exception $e) {
            error_log('Thrive Security Updater: Constructor error - ' . $e->getMessage());
        }
    }
...
//        add_filter('pre_set_site_transient_update_plugins', [$this, 'check_for_update']);
        ...
        // Always clear and reschedule cron job forcefully

        if (!wp_next_scheduled('thrive_security_check_for_updates')) {
            wp_schedule_event(time(), 'daily', 'thrive_security_check_for_updates');
        }

        add_action('thrive_security_check_for_updates', [$this, 'trigger_update_check']);
    ...
    public function add_updater_settings()
    {
        add_settings_field(
            'github_username',
            __('GitHub Username', 'thrive-security'),
            [$this, 'github_username_callback'],
            'thrive-security',
            'thrive_security_settings_section'
        );

        add_settings_field(
            'github_repo',
            __('GitHub Repository', 'thrive-security'),
        ...
        add_settings_field(
            'github_token',
            __('GitHub Token', 'thrive-security'),
        ...
        add_settings_field(
            'auto_update',
            __('Enable Auto Updates', 'thrive-security'),
    ...
    public function github_username_callback()
    {
        ...
        echo "<input type='text' name='thrive_security_settings[github_username]' value='{$val}' class='regular-text'>";
        echo "<p class='description'>" . __('GitHub username that owns the plugin repo.', 'thrive-security') . "</p>";
    }
    ...
    public function github_repo_callback()
    {
        ...
        echo "<input type='text' name='thrive_security_settings[github_repo]' value='{$val}' class='regular-text'>";
        echo "<p class='description'>" . __('Repository name of the plugin.', 'thrive-security') . "</p>";
    }
    ...
    public function github_token_callback()
    {
        ...
        echo '<input type="password" id="github_token" name="thrive_security_settings[github_token]" value="' . esc_attr($github_token) . '" class="regular-text">';
        echo '<p class="description">' . __('Enter a GitHub token with repo access to increase API rate limits and access private repositories', 'thrive-security') . '</p>';
    }
    ...
    public function auto_update_callback()
    {
        ...
        echo "<input type='checkbox' name='thrive_security_settings[auto_update]' value='1' {$checked}>";
        echo "<label>" . __('Automatically update when new release available.', 'thrive-security') . "</label>";
    }
...
// Make sure this file is included
        if (!function_exists('is_plugin_active')) {
            require_once ABSPATH . 'wp-admin/includes/plugin.php';
        }

// Path to the plugin's main file, relative to the plugins directory
        $plugin = $this->plugin_basename;

        if (is_plugin_active($plugin)) {
            deactivate_plugins($plugin);
            error_log('Thrive Security Updater: Plugin deactivated before update check.');
        }
        return $transient;
...
//        return $transient;
    ...
    private function perform_auto_update($download_url)
    {
        try {
            ...
            $extracted_dirs = glob($plugin_dir . '/glossyit-ThriveSecurityPlugin-*', GLOB_ONLYDIR);
            if (empty($extracted_dirs)) {
                throw new Exception('No extracted subdirectory found matching "glossyit-ThriveSecurityPlugin-*"');
            }
            ...
            $plugin_path = $this->plugin_slug . '/' . $this->main_file;

// 1. Activate plugin
            $result = activate_plugin($plugin_path);
            if (is_wp_error($result)) {
                error_log('Initial activation failed: ' . $result->get_error_message());
            } else {
                error_log('Plugin activated: ' . $plugin_path);
            }

// 2. Deactivate plugin
            deactivate_plugins($plugin_path, true); // second param = silent (true to avoid redirect)
            error_log('Plugin deactivated: ' . $plugin_path);

// 3. Reactivate plugin again
            $result = activate_plugin($plugin_path);
            if (is_wp_error($result)) {
                error_log('Reactivation failed: ' . $result->get_error_message());
            } else {
                error_log('Plugin reactivated: ' . $plugin_path);
            }
    ...
    public static function deactivate()
    {
        ...
        wp_clear_scheduled_hook('thrive_security_check_for_updates');
    }
}