...
/**
 * Class THRIVE_SECURITY_LOG_MANAGER
 *
 * Handles logging of blocked plugin/theme events, log file I/O, and daily reporting via email.
 */
class THRIVE_SECURITY_LOG_MANAGER {

    const LOG_FILE = WP_CONTENT_DIR . '/thrive-security-block-log.txt';
    ...
    public static function log(string $type, string $target): void {
        try {
            $ip   = THRIVE_SECURITY_HELPER::get_ip();
            ...
            if (is_writable($log_dir)) {
                ...
                // Fallback to WordPress error log if file is not writable
                error_log('Thrive Security: ' . trim($line));
            }
        } catch (Exception $e) {
            error_log('Thrive Security: Failed to write log entry: ' . $e->getMessage());
        }
    }
    ...
    public static function send_log(): void {
        ...
        <head>
        <meta charset="UTF-8" />
        <title>' . esc_html__('Daily Admin Block Log', THRIVE_SECURITY_TEXT_DOMAIN) . '</title>
        </head>
                    ...
                    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="background:#ffffff; border-radius:8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); overflow:hidden;">
                        ...
                        <tr>
                            <td style="background:#004aad; padding:20px; text-align:center;">
                                <a href="' . $home_url . '" target="_blank" rel="noopener" style="color: #fff; display:inline-block; text-decoration:none;">
                                    <img src="' . $logo_url . '" alt="' . esc_attr__( 'Site Logo', THRIVE_SECURITY_TEXT_DOMAIN ) . '" width="150" style="height:auto; border:none; display:block; margin: 0 auto;" />
                                </a>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td style="padding: 30px 40px 20px; text-align:center; background:#f5f7fa;">
                                <h3 style="margin:0; font-weight:600; color:#004aad; font-size:24px;">' . esc_html__('Daily Admin Block Log', THRIVE_SECURITY_TEXT_DOMAIN) . '</h3>
                            </td>
                        </tr>
        ...
        $body .= '
                        ...
                        <tr>
                            <td style="background:#f5f7fa; padding: 20px 40px; text-align:center; font-size:12px; color:#777;">
                                <p style="margin: 0 0 8px;">' . sprintf(esc_html__('This log file is generated from %s', THRIVE_SECURITY_TEXT_DOMAIN), '<a href="' . esc_url($home_url) . '" target="_blank" rel="noopener noreferrer" style="color:#004aad; text-decoration:none;">' . esc_html($home_url) . '</a>') . '</p>
                                <p style="margin: 0 0 8px;">' . esc_html__('This is an automated message. Please do not reply.', THRIVE_SECURITY_TEXT_DOMAIN) . '</p>
                                <p style="margin: 0;">' . esc_html__('If you have any questions, please contact support.', THRIVE_SECURITY_TEXT_DOMAIN) . '</p>
                            </td>
                        </tr>
        ...
        THRIVE_SECURITY_ALERT_MANAGER::send(__('Daily Admin Block Log From ', THRIVE_SECURITY_TEXT_DOMAIN) . get_bloginfo('name'), $body);
    ...
    public static function download_log(): void {
        if (!file_exists(self::LOG_FILE)) {
            THRIVE_SECURITY_HELPER::display_notice(
                __('Thrive Log file not found.', THRIVE_SECURITY_TEXT_DOMAIN),
                'error',
                admin_url()
            );
        }
        ...
        header('Content-Disposition: attachment; filename="thrive-security-block-log.txt"');
