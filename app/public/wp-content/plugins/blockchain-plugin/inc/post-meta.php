...
add_action( 'after_setup_theme', 'blockchain_plugin_setup_helpers_post_meta' );
function blockchain_plugin_setup_helpers_post_meta() {
	add_image_size( 'blockchain_plugin_featgal_small_thumb', 100, 100, true );

	add_action( 'wp_ajax_blockchain_plugin_featgal_AJAXPreview', 'blockchain_plugin_featgal_AJAXPreview' );
}

function blockchain_plugin_prepare_metabox( $post_type ) {
	wp_nonce_field( basename( __FILE__ ), $post_type . '_nonce' );
}

function blockchain_plugin_can_save_meta( $post_type ) {
	global $post;
...
function blockchain_plugin_metabox_gallery( $gid = 1 ) {
	global $post;
	$post_id = $post->ID;

	blockchain_plugin_featgal_print_meta_html( $post_id, $gid );
}

function blockchain_plugin_metabox_gallery_save( $POST, $gid = 1 ) {
	global $post;
	$post_id = $post->ID;

	blockchain_plugin_featgal_update_meta( $post_id, $POST, $gid );
}

function blockchain_plugin_metabox_input( $fieldname, $label, $params = array() ) {
	global $post;
	...
	if ( ! empty( $label ) ) {
		?><label for="<?php echo esc_attr( $fieldname ); ?>" class="<?php echo esc_attr( $params['label_class'] ); ?>"><?php echo wp_kses( $label, blockchain_plugin_get_allowed_tags() ); ?></label><?php
	}
...
function blockchain_plugin_metabox_textarea( $fieldname, $label, $params = array() ) {
	global $post;
...
function blockchain_plugin_metabox_dropdown( $fieldname, $options, $label, $params = array() ) {
	global $post;
	...
	if ( ! empty( $label ) ) {
		?><label for="<?php echo esc_attr( $fieldname ); ?>"><?php echo wp_kses( $label, blockchain_plugin_get_allowed_tags() ); ?></label><?php
	}
...
function blockchain_plugin_metabox_radio( $fieldname, $optionname, $optionval, $label, $params = array() ) {
	global $post;
	...
	?>
		...
		<label for="<?php echo esc_attr( $optionname ); ?>" class="radio"><?php echo wp_kses( $label, blockchain_plugin_get_allowed_tags() ); ?></label>
	<?php
	echo $params['after'];
}

function blockchain_plugin_metabox_checkbox( $fieldname, $value, $label, $params = array() ) {
	global $post;
	...
	?>
		...
		<label for="<?php echo esc_attr( $fieldname ); ?>"><?php echo wp_kses( $label, blockchain_plugin_get_allowed_tags() ); ?></label>
	<?php
	echo $params['after'];
}

function blockchain_plugin_metabox_open_tab( $title ) {
	?>
...
function blockchain_plugin_metabox_close_tab() {
	?>
		</div>
	</div>
	<?php
...
function blockchain_plugin_metabox_open_collapsible( $title ) {
	?>
	<div class="postbox" style="margin-top:20px">
		<div class="handlediv" title="<?php esc_attr_e( 'Click to toggle', 'blockchain-plugin' ); ?>"><br></div>
		<h3 class="hndle"><?php echo esc_html( $title ); ?></h3>
		<div class="inside">
	<?php
}

function blockchain_plugin_metabox_close_collapsible() {
	?>
		</div>
	</div>
	<?php
...
function blockchain_plugin_metabox_guide( $strings, $params = array() ) {
	$defaults = array(
...
function blockchain_plugin_bind_metabox_to_page_template( $metabox_id, $template_file, $js_var ) {
...
ENDJS;
	...
	wp_add_inline_script( 'blockchain-plugin-post-meta', $js );
}

function blockchain_plugin_bind_metabox_to_post_format( $metabox_id, $post_format, $js_var ) {
 ...
 * @return void
 */
function blockchain_plugin_featgal_print_meta_html( $post_id = false, $gid = 1 ) {
	...
	<div class="ci-media-manager-gallery">
		<input type="button" class="ci-upload-to-gallery button" value="<?php esc_attr_e( 'Add Images', 'blockchain-plugin' ); ?>"/>
		...
		<p><label class="ci-upload-to-gallery-random"><input type="checkbox" name="ci_featured_gallery_rand_<?php echo esc_attr( $gid ); ?>" value="rand" <?php checked( $rand, 'rand' ); ?> /> <?php esc_html_e( 'Randomize order', 'blockchain-plugin' ); ?></label></p>
		<div class="ci-upload-to-gallery-preview group">
			<?php
				$images = blockchain_plugin_featgal_get_images( $ids );
				if ( false !== $images && is_array( $images ) ) {
					foreach ( $images as $image ) {
						?>
						<div class="thumb">
							<img src="<?php echo esc_url( $image['url'] ); ?>" data-img-id="<?php echo esc_attr( $image['id'] ); ?>">
							<a href="#" class="close media-modal-icon" title="<?php esc_attr_e( 'Remove from gallery', 'blockchain-plugin' ); ?>"></a>
						</div>
						<?php
					}
				}
			?>
			<p class="ci-upload-to-gallery-preview-text"><?php esc_html_e( 'Your gallery images will appear here', 'blockchain-plugin' ); ?></p>
		</div>
	</div>
	<?php
}
 ...
 */
function blockchain_plugin_featgal_update_meta( $post_id, $POST, $gid = 1 ) {
	if ( absint( $post_id ) < 1 ) {
		return false;
	}
...
function blockchain_plugin_featgal_get_ids( $post_id = false, $gid = 1 ) {
	if ( false === (bool) $post_id ) {
		global $post;
		$post_id = $post->ID;
	} else {
...
function blockchain_plugin_featgal_get_attachments( $post_id = false, $gid = 1, $extra_args = array() ) {
	if ( false === (bool) $post_id ) {
		global $post;
		$post_id = $post->ID;
	} else {
 ...
 * Hooked to wp_ajax_blockchain_plugin_featgal_AJAXPreview for AJAX updating of the galleries' previews.
 */
function blockchain_plugin_featgal_AJAXPreview() {
	$ids  = isset( $_POST['ids'] ) ? sanitize_text_field( $_POST['ids'] ) : ''; // Input var okay.
	$urls = blockchain_plugin_featgal_get_images( $ids );
	if ( false === $urls ) {
		echo 'FAIL';
 ...
 * @return array|bool
 */
function blockchain_plugin_featgal_get_images( $csv = '' ) {
	$ids = explode( ',', $csv );
	$ids = array_filter( $ids );
	...
	if ( count( $ids ) > 0 ) {
		...
		foreach ( $ids as $id ) {
			$thumb_file = wp_get_attachment_image_url( $id, 'blockchain_plugin_featgal_small_thumb' );

			$file = parse_url( $thumb_file );
			$file = pathinfo( $file['path'] );
			$file = basename( $file['basename'], '.' . $file['extension'] );

			$size = $image_sizes['blockchain_plugin_featgal_small_thumb']['width'] . 'x' . $image_sizes['blockchain_plugin_featgal_small_thumb']['height'];
			...
			$data = array(
				'id'  => $id,
				//'url' => wp_get_attachment_image_url( $id, 'blockchain_plugin_featgal_small_thumb' )
				'url' => $file,
			);
