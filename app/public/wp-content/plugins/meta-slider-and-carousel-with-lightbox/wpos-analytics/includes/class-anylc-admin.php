...
class Wpos_Anylc_Admin {
	...
	public function wpos_monthly_cron_hook_fn() {
		...
		if ( ! empty( $slugs ) ) {
			$this->wpos_process_monthly_data($slugs);
		}
	}
		...
		if ( !empty( $wpos_analytics_module ) ) {
			foreach ( $wpos_analytics_module as $module_key => $module ) {
				...
				if ( !empty( $product_slug ) ) {
					register_rest_route( $product_slug . '/v1', '/analytics/', [
						'methods'  => 'POST',
	...
	public function wpos_handle_analytics_request(WP_REST_Request $request) {
		...
		if ( $site_id && $product_id && $product_slug ) {
			...
			if ( $matching_product ) {
				...
				unset($update_result);
				$this->wpos_process_monthly_data([$matching_product['slug']]);
